...
public final class MobResource implements PreparableReloadListener {
	...
	public CompletableFuture<Void> reload(SharedState state, Executor prepare_exc, PreparationBarrier prep, Executor reload_exc) {
		...
		return CompletableFuture.runAsync(() -> load(manager), prepare_exc).thenCompose(prep::wait).thenRunAsync(this::apply, reload_exc);
	}
